class ScriptRect{
    ...
    void configure(JSONObject config){
        JSONObject p = config.getJSONObject("pos");
        reposition(p.getInt("px"),p.getInt("py"),p.getInt("sx"),p.getInt("sy"));
        
        configure_content(config.getJSONObject("cdata"));
        //more code here
    }
    ...
    JSONObject get_config(){
        ...
        p.setInt("px",round(pos.x/masterGrid.size.x));
        p.setInt("py",round(pos.y/masterGrid.size.y));
        p.setInt("sx",round(size.x/masterGrid.size.x));
        p.setInt("sy",round(size.y/masterGrid.size.y));
        config.setString("module",module_id);
        JSONObject cdata = export_content_config();
        
        config.setJSONObject("pos",p);
        config.setJSONObject("cdata",cdata);
        return config;
    }
